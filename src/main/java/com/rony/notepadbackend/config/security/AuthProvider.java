...
public class AuthProvider extends AbstractUserDetailsAuthenticationProvider {
    ...
    protected UserDetails retrieveUser(String username, UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken) throws AuthenticationException {
        ...
        if (token.isEmpty()) {
            return new org.springframework.security.core.userdetails.User (username, "", AuthorityUtils.createAuthorityList("ROLE_ANONYMOUS"));
        }
        ...
        if (optionalMarkdownUserModel.isPresent()) {
            ...
            return new org.springframework.security.core.userdetails.User(username, "",
                    AuthorityUtils.createAuthorityList(user.getRoles().stream().map(s -> "ROLE_" + s).toArray(String[]::new))
            );
        }
